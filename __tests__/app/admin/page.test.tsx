...
describe('AdminPage', () => {
  ...
  it('elimina un pato y muestra mensaje de éxito', () => {
    ...
    expect(
      screen.getByText('Especie eliminada exitosamente')
    ).toBeInTheDocument();
  });
  ...
  it('agrega una especie nueva y muestra mensaje de éxito', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Nombre'), {
      target: { value: 'Nuevo Pato' },
    });
    fireEvent.change(screen.getByLabelText('Nombre Científico'), {
      target: { value: 'Anas nuevo' },
    });
    fireEvent.change(screen.getByLabelText('Especie'), {
      target: { value: 'Nueva' },
    });
    fireEvent.change(screen.getByLabelText('URL de Imagen'), {
      target: { value: '/nuevo.jpg' },
    });
    fireEvent.change(screen.getByLabelText('Descripción'), {
      target: { value: 'Desc' },
    });
    fireEvent.change(screen.getByLabelText('Comportamiento'), {
      target: { value: 'Activo' },
    });
    fireEvent.change(screen.getByLabelText('Hábitat'), {
      target: { value: 'Ríos' },
    });
    fireEvent.change(screen.getByLabelText('Plumaje'), {
      target: { value: 'Gris' },
    });
    fireEvent.change(screen.getByLabelText('Alimentación'), {
      target: { value: 'Peces' },
    });
    fireEvent.change(screen.getByLabelText('URL de Sonido'), {
      target: { value: '/nuevo.mp3' },
    });
    fireEvent.click(screen.getByRole('button', { name: /Agregar/i }));
    await waitFor(() => {
      expect(PatoService.addPato).toHaveBeenCalled();
      expect(
        screen.getByText('Nueva especie agregada exitosamente')
      ).toBeInTheDocument();
    });
  });
});
